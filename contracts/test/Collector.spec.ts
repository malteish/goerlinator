...
describe("Collector.sol", () => {
  it("claim flows", async () => {
    ...
    let merkleTree = new MerkleTree(addrs, keccak256, {
      hashLeaves: true,
      sortPairs: true,
    });
    ...
    let leaf = keccak256(randomAddr);
    let proof = merkleTree.getHexProof(leaf);
    await expect(
      deploy.collect(proof, "I am a graffiti", randomAddr)
    ).to.be.revertedWith("Failed merkle proof");

    // Transfer fails for insufficient funds
    deploy = deploy.connect(signers[1]);
    let signer1Addr = await signers[1].getAddress();
    leaf = keccak256(signer1Addr);
    proof = merkleTree.getHexProof(leaf);
    await expect(
      deploy.collect(proof, "I am a graffiti", signer1Addr)
    ...
    await expect(deploy.collect(proof, "I am a graffiti", signer1Addr)).to.not
      .be.reverted;
    ...
    await expect(
      deploy.collect(proof, "I a a graffiti", signer1Addr)
  ...
  it("admin permissions", async () => {
    ...
    await expect(
      deploy.connect(signers[1]).adminWithdraw(WITHDRAW_AMOUNT)
    ).to.be.revertedWith("Ownable: caller is not the owner");
    await expect(deploy.connect(signers[0]).adminWithdraw(WITHDRAW_AMOUNT)).to
      .not.be.reverted;
    ...
    await expect(
      deploy.connect(signers[1]).adminUpdateRoot(root)
    ).to.be.revertedWith("Ownable: caller is not the owner");
    await expect(deploy.connect(signers[0]).adminUpdateRoot(root)).to.not.be
      .reverted;
  });
});
