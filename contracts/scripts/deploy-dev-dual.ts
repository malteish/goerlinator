...
let CLAIM_AMOUNT = utils.parseEther("0.42"); // ETH amount per claim
...
function deployAnvils() {
  ...
  Configs.forEach((config, index) => {
    let process = child.exec(
      `anvil --chain-id ${config.chain_id} --port ${config.port} --block-time 10`,
      (error, stdout, stderr) => {
  ...
  Configs.forEach(async (config, index) => {
    ...
    devAddresses.forEach(async (address) => {
      await provider.send("anvil_setBalance", [
        address,
        DEV_START_BALANCE._hex,
      ]);
    });

    let deploySigner = new Wallet(
